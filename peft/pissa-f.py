...
class Pissa(nn.Module):
    ...
    def __init__(
        ...
        with torch.no_grad():
            self.linear_a_q.weight.copy_(Vh_q_r)
            self.linear_b_q.weight.copy_(U_q_r @ torch.diag(S_q_r))
            
            self.linear_a_v.weight.copy_(Vh_v_r)
            self.linear_b_v.weight.copy_(U_v_r @ torch.diag(S_v_r))
